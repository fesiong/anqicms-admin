export default {
  ...
  'plugin.aigenerate.demand.description': '可以定义所有AI写作文章的统一要求，不超过200字符。 默认留空即可',
  ...
  'plugin.aigenerate.openai.description': 'OpenAI Key 一般以 sk- 开头，可以添加多个key，程序会每次会随机选择一个key使用。',
  ...
  'plugin.aigenerate.default-category.description': '如果关键词没设置分类，则采集到的文章默认会被随机归类到其中一个分类下,必须设置一个分类否则无法正常采集',
  ...
  'plugin.aigenerate.replace.tips2': '替换规则支持正则表达式，如果你对正则表达式熟悉，并且通过普通文本无法达成替换需求的，可以尝试使用正则表达式规则来完成替换。',
  'plugin.aigenerate.replace.tips3': '正则表达式规则为：由 { 开始，并以 } 结束，中间书写规则代码，如 {[0-9]+} 代表匹配连续的数字。',
  ...
  'plugin.aigenerate.replace.notice': '注意：正则表达式规则书写不当很容易造成错误的替换效果，如微信号规则，会同时影响到邮箱地址、网址的完整性。请谨慎使用。',
  ...
  'plugin.aigenerate.image.category.description': '会自动从指定的图片资源分类中选择图片。如果选择尝试关键词匹配图片名称，则会尝试将文章关键词和图片名称进行匹配，如果匹配成功则使用图片。',
  ...
  'plugin.aigenerate.tips2': 'AI自动写作会自动调用关键词库中的关键词来完成写作，每一个关键词写作一篇文章。请保证关键词库中的文章数量足够。',
  'plugin.aigenerate.tips3': 'AI自动写作和文章采集功能共用关键词库，关键词已采集过文章的话，不再用来AI写作。',
  ...
  'plugin.anchor.link.description': '支持相对链接和绝对连接，如：/a/123.html 或 https://www.anqicms.com/',
  ...
  'plugin.backup.cleanup.tips1': '该操作会删除全部文章。为了安全起见，请你务必先执行备份，以防不备之需。',
  ...
  'plugin.backup.tips': '说明：如果备份文件过大，需要下载备份文件，请使用FTP工具等下载备份文件，备份文件在网站根目录/data/backup/ 目录下。',
  ...
  'plugin.collector.mode.description': '文章采集模式，会按原文采集整篇文章；问答组合模式，会从搜索问答列表中采集并组合成文章。',
  'plugin.collector.source': '自定义来源',
  'plugin.collector.source.description': "文章采集可用，注意自定义来源格式必须是一个搜索列表，搜索的关键词用 %s 表示，如搜索链接是：https://cn.bing.com/search?q=安企CMS ，则将 '安企CMS' 替换为 '%s' 后为：https://cn.bing.com/search?q=%s",
  'plugin.collector.category.description': '如果关键词没设置分类，则采集到的文章默认会被归类到这个分类下,',
  ...
  'plugin.collector.min-content.description': '采集文章的时候，文章内容字数少于指定的字数，则不会采集',
  ...
  'plugin.collector.translate.description': '翻译需要付费。注意：AI改写和翻译不能同时启用，否则结果会出错',
  ...
  'plugin.collector.content-exclude-line.tips': '采集文章的时候，内容出现这些词的那一行，将会被移除',
  ...
  'plugin.collector.tips': '采集文章需要先设置核心关键词，请检查“关键词库管理”功能，并添加相应的关键词。',
