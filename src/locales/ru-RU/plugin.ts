export default {
  ...
  'plugin.aigenerate.demand.description': 'Можно определить единые требования для всех статей по написанию ИИ, не более 200 символов. Оставьте это поле пустым по умолчанию',
  ...
  'plugin.aigenerate.source.description': 'Заявление: Только зарубежные веб-сайты, созданные с использованием Anqi CMS, могут подготовить собственный OpenAIKey.',
  'plugin.aigenerate.source.check-openai': 'Проверьте интерфейс OpenAI',
  'plugin.aigenerate.openai.description': 'Ключ OpenAI обычно начинается с sk-, вы можете добавить несколько ключей, и программа каждый раз будет случайным образом выбирать один ключ для использования.',
  ...
  'plugin.aigenerate.default-category.description': 'Если ключевые слова не распределены по категориям, собранные статьи по умолчанию будут случайным образом отнесены к одной из категорий. Необходимо установить категорию, иначе собранные статьи не смогут быть собраны нормально.',
  ...
  'plugin.aigenerate.start-time.description': 'Пожалуйста, введите число от 0 до 23, 0 означает отсутствие ограничений.',
  'plugin.aigenerate.end-time': 'конец дня',
  'plugin.aigenerate.end-time.placeholder': 'По умолчанию заканчивается в 22:00.',
  'plugin.aigenerate.end-time.description': 'Пожалуйста, введите число от 0 до 23, 0 означает отсутствие ограничений.',
  'plugin.aigenerate.daily-limit': 'Ежедневные выпуски',
  'plugin.aigenerate.daily-limit.description': 'Максимальное количество статей, публикуемых в день, 0 означает отсутствие ограничений.',
  ...
  'plugin.aigenerate.replace.tips1': 'Отредактируйте пары ключевых слов, которые необходимо заменить, и замена будет выполнена автоматически при публикации документа.',
  'plugin.aigenerate.replace.tips2': 'Правила замены поддерживают регулярные выражения. Если вы знакомы с регулярными выражениями и не можете добиться замены с помощью обычного текста, вы можете попробовать использовать правила регулярных выражений для завершения замены.',
  'plugin.aigenerate.replace.tips3': 'Правила регулярных выражений: начинаются с { и заканчиваются }, а код правила пишется посередине, например {[0-9]+}, чтобы соответствовать последовательным числам.',
  'plugin.aigenerate.replace.rules': 'Некоторые встроенные правила можно использовать быстро. Встроенные:',
  ...
  'plugin.aigenerate.replace.notice': 'Примечание. Неправильное написание правил регулярных выражений может легко привести к неправильным эффектам замены. Например, правила WeChat ID повлияют на целостность адресов электронной почты и URL-адресов. Пожалуйста, используйте с осторожностью.',
  ...
  'plugin.aigenerate.start.description': 'При этом немедленно начнется выполнение операции записи ИИ.',
  'plugin.aigenerate.image.category': 'Классификация изображений',
  'plugin.aigenerate.image.category.description': 'Изображения будут автоматически выбраны из указанной категории ресурсов изображений. Если вы решите попробовать сопоставление ключевых слов с названием изображения, оно попытается сопоставить ключевые слова статьи с названием изображения, и если сопоставление окажется успешным, будет использовано изображение.',
  'plugin.aigenerate.image.category.default': 'Фотографии без категорий',
  'plugin.aigenerate.image.category.all': 'Все фотографии',
  'plugin.aigenerate.image.category.match': 'Попробуйте ключевое слово, соответствующее названию изображения.',
  ...
  'plugin.aigenerate.tips1': 'Автоматическое письмо ИИ вызовет интерфейс записи ИИ для записи, что требует оплаты.',
  'plugin.aigenerate.tips2': 'Автоматическое написание ИИ автоматически вызовет ключевые слова в библиотеке ключевых слов для завершения написания и напишет статью для каждого ключевого слова. Убедитесь, что количество статей в базе данных ключевых слов достаточно.',
  'plugin.aigenerate.tips3': 'Функции автоматического написания статей с помощью ИИ и сбора статей используют общую библиотеку ключевых слов. Если ключевые слова были собраны для статей, они больше не будут использоваться для написания ИИ.',
  ...
  'plugin.anchor.link.description': 'Поддерживает относительные и абсолютные ссылки, например: /a/123.html или https://www.anqicms.com/.',
  'plugin.anchor.weight': 'Вес текста привязки',
  'plugin.anchor.weight.description': 'Введите число от 0 до 9. Чем больше число, тем выше вес. Более высокие веса имеют приоритет при замене.',
  'plugin.anchor.import': 'Импортировать текст привязки',
  'plugin.anchor.import.description': 'Примечание. Для загрузки и импорта поддерживаются только файлы в формате CSV.',
  ...
  'plugin.anchor.density.description': 'Например: каждые 100 слов для замены текста привязки заполняйте 100, по умолчанию — 100.',
  ...
  'plugin.anchor.extract.description': 'Выберите способ извлечения ключевых слов привязки текста из тегов ключевых слов контента.',
  ...
  'plugin.backup.restore.confirm': 'Вы уверены, что хотите восстановить данные из текущей резервной копии?',
  'plugin.backup.restore.content': 'После восстановления существующие данные будут заменены текущими данными резервной копии. Пожалуйста, действуйте осторожно.',
  ...
  'plugin.backup.cleanup.tips1': 'Эта операция удалит все статьи. В целях безопасности обязательно сначала выполните резервное копирование на случай непредвиденных обстоятельств.',
  'plugin.backup.cleanup.tips2': 'Папка «Загрузки» не очищается по умолчанию. Если вам нужно ее очистить, проверьте ее.',
  ...
  'plugin.backup.tips': 'Примечание. Если файл резервной копии слишком велик и вам необходимо загрузить файл резервной копии, используйте инструменты FTP для загрузки файла резервной копии. Файл резервной копии находится в каталоге /data/backup/ корневого каталога веб-сайта.',
  ...
  'plugin.collector.mode.description': 'Режим сбора статей соберет всю статью в соответствии с исходным текстом; режим комбинирования вопросов и ответов соберет и объединит ее в статьи из поискового списка вопросов и ответов.',
  'plugin.collector.source': 'пользовательские источники',
  'plugin.collector.source.description': 'Доступен сборник статей. Обратите внимание, что пользовательский формат источника должен быть списком поиска, а ключевые слова для поиска представлены %s. Например, ссылка для поиска: https://cn.bing.com/search?q=. Anqi CMS, затем "Anqi CMS" заменяется на "%s", а затем: https://cn.bing.com/search?q=%s',
  'plugin.collector.category.description': 'Если для ключевого слова не задана категория, собранные статьи по умолчанию будут отнесены к этой категории.',
  'plugin.collector.category.notice': 'Категория должна быть установлена, иначе нормальный сбор будет невозможен.',
  ...
  'plugin.collector.min-title.description': 'При сборе статьи, если количество слов в заголовке меньше указанного количества слов, она не будет собрана.',
  ...
  'plugin.collector.min-content.description': 'При сборе статей, если количество слов в содержании статьи меньше указанного количества слов, она не будет собрана.',
  ...
  'plugin.collector.pseudo.description': 'Переписывание ИИ поддерживает только сбор статей и комбинации вопросов и ответов. Требуется плата.',
  'plugin.collector.translate': 'Стоит ли переводить',
  'plugin.collector.translate.no': 'нет',
  'plugin.collector.translate.yes': 'Переводить',
  'plugin.collector.translate.description': 'За перевод взимается плата. Примечание. Переписывание и перевод ИИ нельзя включить одновременно, иначе результаты будут неверными.',
  ...
  'plugin.collector.daily-limit.description': 'Максимальное количество статей, собираемых в день, 0 означает отсутствие ограничений.',
  ...
  'plugin.collector.title-exclude.tips': 'Если при сборе статей эти ключевые слова встречаются в заголовке, они не будут собраны.',
  'plugin.collector.title-prefix': 'Исключить слова в начале заголовка',
  'plugin.collector.title-prefix.tips': 'Если при сборе статей эти ключевые слова появляются в начале заголовка, они не будут собраны.',
  'plugin.collector.title-suffix': 'Исключить слова в конце заголовка',
  'plugin.collector.title-suffix.tips': 'Если при сборе статей эти ключевые слова появляются в конце заголовка, они не будут собраны.',
  'plugin.collector.content-exclude-line': 'строка игнорирования содержимого',
  'plugin.collector.content-exclude-line.tips': 'При сборе статей строки, в которых встречаются эти слова, будут удалены.',
  'plugin.collector.content-exclude': 'Исключение контента',
  'plugin.collector.content-exclude.tips': 'При сборе статей, если эти слова встречаются в содержании, вся статья будет удалена.',
  'plugin.collector.link-exclude': 'Ссылка проигнорирована',
  'plugin.collector.link-exclude.tips': 'При сборе статей, если эти ключевые слова встречаются в ссылке, они не будут собраны.',
  'plugin.collector.start': 'Начать сбор вручную',
  'plugin.collector.start.confirm': 'Вы уверены, что хотите начать коллекционировать?',
  'plugin.collector.start.content': 'Это немедленно начнет выполнение операции задачи сбора данных.',
  'plugin.collector.tips': 'Чтобы собирать статьи, вам необходимо сначала установить основные ключевые слова. Пожалуйста, проверьте функцию «Управление базой данных ключевых слов» и добавьте соответствующие ключевые слова.',
  ...
  'plugin.fileupload.upload.support': 'Примечание. Разрешается загружать только файлы подтверждения в формате txt/htm/html/xml.',
  ...
  'plugin.finance.withdraw.confirm.content': 'Это эквивалентно подаче заявки на вывод средств со стороны пользователя.',
  ...
  'plugin.finance.withdraw.finish.confirm': 'Вы уверены, что хотите завершить вывод средств вручную?',
  'plugin.finance.withdraw.finish.content': 'Если вы заплатили пользователю в автономном режиме, вы можете нажать здесь, чтобы завершить оплату.',
  ...
  'plugin.fulltext.tips': 'После включения полнотекстового поиска вы можете осуществлять поиск по содержимому документа. Однако полнотекстовый поиск будет занимать много памяти сервера. Если на вашем сервере мало памяти, не рекомендуется включать полнотекстовый поиск.',
  ...
  'plugin.group.level.description': 'Например, обычные участники имеют уровень 0, промежуточные участники — уровень 1, старшие члены — уровень 2, основные члены — уровень 3 и т. д. Заполните числа.',
  'plugin.group.price': 'Цена группы пользователей',
  'plugin.group.price.suffix': 'точка',
  'plugin.group.price.description': 'Цена, которую необходимо заплатить за приобретение VIP для этой группы пользователей. Обратите внимание, что единица измерения — цент. Например, 1 юань, введите здесь 100.',
  'plugin.group.expire_day': 'Срок действия группы пользователей',
  'plugin.group.expire_day.suffix': 'небо',
  'plugin.group.expire_day.description': 'После покупки VIP он будет действителен в течение скольких дней. Введите 365 на 1 год. По истечении срока действия он вернется в первую группу пользователей.',
  'plugin.group.share_reward': 'Срок действия группы пользователей',
  'plugin.group.share_reward.description': 'Рекомендуется установить 5%-20, десятичная точка не может быть установлена. Приоритет коэффициента комиссии: коэффициент комиссии, установленный продуктом > коэффициент комиссии группы пользователей > коэффициент комиссии по умолчанию.',
  'plugin.group.parent_reward': 'Коэффициент вознаграждения за приглашение',
  'plugin.group.parent_reward.description': 'Рекомендуется установить 1%-5%, десятичная точка не может быть установлена. Превосходное соотношение комиссий. Дистрибьютор a приглашает b стать дистрибьютором, и b становится следующим продавцом a. Когда b успешно продвигает заказ, b может получить комиссию за распространение, а a получает только вознаграждение за приглашение.',
  'plugin.group.discount': 'Скидка пользователя',
  'plugin.group.discount.description': 'Рекомендуется установить 90%-100%. После того, как пользователи перейдут на страницу по ссылке, предоставленной дистрибьютором, они смогут воспользоваться скидкой при оформлении заказа.',
  ...
  'plugin.guestbook.reply.required': 'Сначала настройте напоминание по электронной почте и найдите в функции «Напоминание по электронной почте».',
  'plugin.guestbook.replysubmit.required': 'Пожалуйста, заполните заголовок и содержание электронного письма.',
  ...
  'plugin.guestbook.field.delete.confirm.content': 'Вы можете восстановить, обновив страницу перед сохранением.',
  ...
  'plugin.guestbook.return-message.description': 'Подсказка, которую пользователи видят после отправки сообщения. Например: Спасибо за ваше сообщение!',
  ...
  'plugin.htmlcache.generate.all.confirm': 'Вы уверены, что хотите создать статический кеш для всего сайта?',
  'plugin.htmlcache.generate.home.confirm': 'Вы уверены, что хотите создать статический кеш главной страницы?',
  'plugin.htmlcache.generate.category.confirm': 'Вы уверены, что хотите создать статический кеш столбца?',
  'plugin.htmlcache.generate.archive.confirm': 'Вы уверены, что хотите создать статический кэш документа?',
  'plugin.htmlcache.generate.tag.confirm': 'Вы уверены, что хотите создать статический кеш тегов?',
  'plugin.htmlcache.clean.confirm': 'Вы уверены, что хотите очистить статический кеш всего сайта? Если кэшированных файлов много, это может занять много времени.',
  'plugin.htmlcache.clean.confirm.content': 'Эта операция очищает только файлы локального кэша сервера и не может очистить статические файлы сервера.',
  'plugin.htmlcache.clean.success': 'Очистка прошла успешно',
  'plugin.htmlcache.push.all.confirm': 'Вы уверены, что хотите отправить все статические файлы на статический сервер?',
  'plugin.htmlcache.push.all.confirm.content': 'Он доступен только в том случае, если настроен статический сервер. Полная передача занимает много времени. Если глобальные изменения не вносятся, можно использовать инкрементную отправку.',
  'plugin.htmlcache.push.addon.confirm': 'Вы уверены, что хотите постепенно отправлять статические файлы на статический сервер?',
  'plugin.htmlcache.push.addon.confirm.content': 'Доступно только в том случае, если настроен статический сервер. Инкрементная передача будет отправлять только обновленные файлы статического кэша.',
  ...
  'plugin.htmlcache.index-time.description': 'Если вы заполните 0 секунд, оно не будет кэшироваться.',
  ...
  'plugin.htmlcache.tencent.bucket-url.placeholder': 'Например: https://aa-1257021234.cos.ap-guangzhou.myqcloud.com',
  ...
  'plugin.htmlcache.ftp.tips': 'Примечание. После тестирования PureFtp, поставляемый с Pagoda, нельзя использовать нормально.',
  ...
  'plugin.htmlcache.ssh.or-key.description': 'Если ваш SSH-сервер использует ключ для входа в систему, загрузите его.',
  ...
  'plugin.htmlcache.description.1': 'После включения статического кэширования страниц домашняя страница, страница списка и страница сведений будут кэшироваться для ускорения открытия веб-сайта, но для хранения файлов кэша потребуется больше места на сервере.',
  'plugin.htmlcache.description.2': 'Если вам нужно включить статический веб-сайт, тип шаблона должен быть адаптивным. Чтобы открыть статический веб-сайт, вам необходимо заполнить информацию о сервере статического веб-сайта. После успешной связи система автоматически передаст статическую страницу на сервер статического веб-сайта.',
  'plugin.htmlcache.description.3': 'Прежде чем включать статический веб-сайт, вам необходимо включить кеширование статических страниц. После включения статического веб-сайта поиск, сообщение, комментарий, переход 301 и другие функции, требующие отправки данных на серверную часть, будут недействительны, и веб-сайт будет иметь только эффекты отображения.',
  'plugin.htmlcache.description.4': 'После включения статического веб-сайта следующие операции не будут автоматически перегенерированы, и операции по созданию статических страниц необходимо выполнять вручную: Настройка шаблонов (изменение шаблонов, включение шаблонов), Изменение настроек фона (глобальные настройки, настройки контента, контактная информация, навигация). и т.д.), модифицированные псевдостатические правила и другие изменения, влияющие на глобальную ситуацию.',
  ...
  'plugin.importapi.token.confirm.content': 'После обновления исходный токен становится недействительным, для работы используйте новый адрес API.',
  'plugin.importapi.token.copy.success': 'Скопировано успешно',
  'plugin.importapi.tips': 'Контент, созданный с помощью сторонних платформ, таких как написание ИИ, можно импортировать в эту систему через API.',
  ...
  'plugin.importapi.field.archive-id': 'Идентификатор документа, автоматически генерируется по умолчанию',
  ...
  'plugin.importapi.field.url-token': 'Пользовательский псевдоним URL-адреса, поддерживает только цифры и английские буквы.',
  'plugin.importapi.field.images': 'Изображения статьи могут быть установлены до 9 изображений.',
  'plugin.importapi.field.logo': 'Миниатюра документа может представлять собой абсолютный адрес, например: https://www.anqicms.com/logo.png, или относительный адрес, например: /logo.png.',
  'plugin.importapi.field.publish-time': 'Формат: 2006-01-02 15:04:05 Время выпуска документа может быть в будущем. Если это произойдет, документ не будет официально выпущен, пока время не истечет.',
  'plugin.importapi.field.tag': 'Тег документа. Несколько тегов разделяются английскими запятыми, например: aaa, bbb, ccc.',
  'plugin.importapi.field.diy': 'Другие настраиваемые поля',
  'plugin.importapi.field.diy.remark': 'Если вы передаете другие настраиваемые поля и они существуют в таблице документа, они также поддерживаются.',
  'plugin.importapi.field.draft': 'Сохранять ли в черновик. Поддерживаемые значения: false|true. Если указано значение true, опубликованный документ будет сохранен в черновике.',
  'plugin.importapi.field.cover': 'Следует ли перезаписывать документы с тем же заголовком и идентификатором, если они существуют. Поддерживаемые значения: false|true. Если указано значение true, последнее содержимое будет перезаписано. При значении false будет выдано сообщение об ошибке.',
  ...
  'plugin.importapi.category-api.module-id': 'Идентификатор модели классификации, которую необходимо получить. Введите число. Поддерживаемые значения: 0 = все, 1,2... соответствующий идентификатор модели.',
  ...
  'plugin.importapi.train-mopdule.support-version.text': 'Поддержка сборщика локомотивов версии 9.0 или выше для импорта и использования модуля выпуска.',
  ...
  'plugin.importapi.token.new.description': 'Токен обычно состоит из комбинации цифр и букв длиной более 10 и менее 128 цифр.',
  'plugin.interference.isopen': 'Включите сбор кода защиты от помех',
  'plugin.interference.isopen.description': 'Следующие настройки действуют только в том случае, если эта функция включена.',
  ...
  'plugin.keyword.batch-import.tips': 'Примечание. Для загрузки и импорта поддерживаются только файлы в формате CSV.',
  ...
  'plugin.keyword.title.placeholder': 'Поддерживает пакетное добавление, по одному ключевому слову в строке.',
  ...
  'plugin.keyword.dig-setting.max-count.description': 'Если выбрано автоматическое расширение слов, количество расширений слов будет действительным.',
  ...
  'plugin.keyword.dig-setting.title-exclude.description': 'При расширении слов, если эти ключевые слова встречаются в ключевых словах, они не будут собраны.',
  'plugin.keyword.dig-setting.replace': 'Замена ключевых слов',
  'plugin.keyword.dig-setting.replace.tips1': 'Отредактируйте пары ключевых слов, которые необходимо заменить, и замена будет выполнена автоматически при раскрытии слов.',
  ...
  'plugin.keyword.aigenerate.confirm': 'Вы уверены, что хотите выполнить операции записи ИИ по этому ключевому слову?',
  'plugin.keyword.aigenerate.content': 'Автоматическое написание AI требует оплаты. Убедитесь, что вы привязали свою учетную запись Anqi.',
  'plugin.keyword.aigenerate.doing': 'Создание',
  'plugin.keyword.cleanup.confirm': 'Вы уверены, что хотите удалить все ключевые слова?',
  'plugin.keyword.cleanup.content': 'Эта операция удалит все ключевые слова и не сможет быть восстановлена. Действуйте осторожно.',
  ...
  'plugin.link.field.nofollow': 'Добавлять ли nofollow, необязательные значения: 0 — не добавлять, 1 — добавлять.',
  'plugin.link.field.back-link': 'Страница с противоположной ссылкой',
  'plugin.link.field.back-link.description': 'URL-адрес страницы, на которой другая сторона разместила ссылку на этот сайт.',
  'plugin.link.field.self-title': 'мои ключевые слова',
  'plugin.link.field.self-title.description': 'Ключевые слова, которые я размещаю на странице другого абонента',
  'plugin.link.field.self-link': 'моя ссылка',
  'plugin.link.field.self-link.description': 'Ссылка, которую я разместил на странице другой стороны',
  'plugin.link.field.contact': 'Контактная информация другой стороны',
  'plugin.link.field.contact.description': 'Заполните QQ, WeChat, номер телефона и т. д., чтобы облегчить последующий контакт.',
  ...
  'plugin.material.import.clear': 'Вы уверены, что хотите удалить материалы контента, выбранные для загрузки?',
  ...
  'plugin.material.import.submit.tips.after': 'Для этого материала не выбран раздел. Хотите продолжить отправку?',
  ...
  'plugin.material.import.batch-add.tips': 'Примечание. Вы можете загружать статьи, хранящиеся в формате txt или html.',
  ...
  'plugin.material.import.paste.description': 'По умолчанию контент будет фильтровать все теги HTML и сохранять только текст. Если вам нужно сохранить html-теги, пожалуйста, проверьте',
  ...
  'plugin.order.setting.progress.description': 'Обычные транзакции требуют, чтобы пользователь подтвердил получение или завершил заказ по истечении срока действия. Транзакция завершается непосредственно после оплаты пользователем и завершения заказа.',
  ...
  'plugin.order.apply-refund.confirm': 'Вы уверены, что хотите подать заявку на возврат средств за этот заказ?',
  'plugin.order.apply-refund.content': 'После возврата средства будут возвращены по первоначальному маршруту.',
  ...
  'plugin.push.tips': 'Функция push в поисковой системе поддерживает активную отправку с помощью поиска Baidu и поиска Bing. Хотя другие поисковые системы не имеют функции активной отправки, некоторые поисковые системы все равно могут использовать JS push.',
  ...
  'plugin.push.baidu.api-link.description': 'Например: http://data.zz.baidu.com/urls?site=https://www.anqicms.com&token=DTHpH8Xn99BrJLBY.',
  'plugin.push.bing.api-link.description': 'Например: http://data.zz.baidu.com/urls?site=https://www.anqicms.com&token=DT Например: https://ssl.bing.com/webmaster/api.svc/json /SubmitUrlbatch ?apikey=sampleapikeyEDECC1EA4AE341CC8B6 (обратите внимание, что этот API-ключ задается в настройках в правом верхнем углу инструмента Bing)',
  'plugin.push.google': 'Ключ аккаунта Google в формате JSON',
  'plugin.push.google.json': 'JSON-контент',
  'plugin.push.google.description': 'Не доступен внутри страны. Чтобы получить JSON, обратитесь к документу: https://www.anqicms.com/google-indexing-help.html.',
  'plugin.push.other-js': '360/Toutiao и другие JS автоматически отправляют',
  'plugin.push.other-js.add': 'Добавить JS-код',
  'plugin.push.other-js.tips1': 'Вы можете разместить коды JS, такие как автоматическая отправка Baidu JS, автоматическое включение 360° и автоматическое включение Toutiao.',
  'plugin.push.other-js.tips2': 'Эти коды необходимо вызывать вручную в шаблоне. Добавьте код `{{- плагинJsCode|safe }}` в конце общедоступного шаблона для вызова.',
  'plugin.push.other-js.tips3': 'Всплывающие окна, такие как сообщения/комментарии, автоматически загружают эти JS-коды.',
  ...
  'plugin.redirect.import.tips': 'Примечание. Для загрузки и импорта поддерживаются только файлы в формате CSV.',
  ...
  'plugin.redirect.from-url.description': 'Это может быть абсолютный адрес, начинающийся с «http(https)», или относительный адрес, начинающийся с «/».',
  ...
  'plugin.replace.tips': 'Замена всего сайта — это сложная операция, поэтому при замене могут возникнуть ошибки. Перед заменой рекомендуется выполнить резервное копирование содержимого.',
  ...
  'plugin.retailer.allow-self.description': 'Если комиссия за самозакупку включена, дистрибьютор может получить соответствующую комиссию, если он сам приобретет распространяемый товар. Если она отключена, дистрибьютор не сможет получить комиссию, если он сам приобретет распространяемый товар. Если вы автоматически становитесь дистрибьютором, не включайте комиссию за самостоятельную покупку.',
  ...
  'plugin.retailer.become-retailer.description': 'Если вы выбираете ручную обработку, вам необходимо настроить ее в управлении пользователями.',
  'plugin.retailer.cancel.confirm': 'Вы уверены, что хотите отменить квалификацию дистрибьютора этого пользователя?',
  'plugin.retailer.cancel.content': 'Если порог дистрибьютора должен автоматически стать дистрибьютором, отмена будет недействительной.',
  ...
  'plugin.rewrite.formula5': 'Вариант 5: Пользовательский режим (расширенный режим, используйте его с осторожностью, если он настроен неправильно, главная страница не откроется)',
  ...
  'plugin.rewrite.setting.diy.tips': 'Скопируйте следующие правила в поле ввода для изменения. Всего имеется 6 строк, а именно сведения о документе, список документов, домашняя страница модели, страница, список тегов и сведения о тегах. === и предыдущую часть нельзя изменить.',
  'plugin.rewrite.variable.tips': 'Переменные заключаются в фигурные скобки `{}`, например `{id}`. Доступные переменные: идентификатор данных `{id}`; имя пользовательской ссылки классификации документа `{filename}`; имя пользовательской ссылки многоуровневой классификации `{multicatname}`, `{ Только один из можно использовать multicatname}` и `{catname}`; идентификатор классификации `{catid}`; имя таблицы модели `{module}`; год `{year}`, месяц `{month}`, день `{day} `; , час `{hour}`, минута `{минута}`, секунда `{секунда}`, год, месяц, день, час, минута и секунда доступны только в архиве с номером страницы пейджинга `{page}`, пейджинг; необходимо помещать в круглые скобки, например: `(/{page})` .',
  'plugin.rewrite.formula.direct1': 'Готовое решение 1',
  'plugin.rewrite.formula.direct2': 'Решение 2, которое можно использовать напрямую',
  'plugin.rewrite.formula.direct3': 'Решение 3, которое можно использовать напрямую',
  'plugin.robots.tips.before': 'Роботы — это конфигурация веб-сайта, которая сообщает роботам поисковых систем, какие страницы можно сканировать, а какие нет. Вопрос:',
  'plugin.robots.tips.after': 'Формат файла robots.txt',
  'plugin.robots.content': 'Содержание роботов',
  'plugin.robots.content.tips1': '1. Robots.txt может сообщить Baidu, какие страницы вашего веб-сайта можно сканировать, а какие нельзя.',
  'plugin.robots.content.tips2': '2. Вы можете использовать инструмент Robots для создания, проверки и обновления файла robots.txt.',
  ...
  'plugin.sendmail.port.description': 'Порт сервера по умолчанию — 25, порт по умолчанию при использовании протокола SSL — 465, а порт по умолчанию при использовании протокола TLS — 587. Подробные параметры уточняйте у своего поставщика услуг электронной почты.',
  'plugin.sendmail.account': 'SMTP-аккаунт',
  'plugin.sendmail.account.description': 'По умолчанию используется учетная запись электронной почты, например адрес электронной почты QQ, например 123456@qq.com.',
  'plugin.sendmail.password': 'SMTP-пароль',
  'plugin.sendmail.password.description': 'Код авторизации генерируется в настройках электронной почты.',
  ...
  'plugin.sendmail.recipient.description': 'По умолчанию оно отправляется отправителю. Если вам нужно отправить его другим людям, заполните его здесь. Разделите нескольких получателей запятыми.',
  ...
  'plugin.sendmail.auto-reply.description': 'Если автоматический ответ клиентам включен, когда клиент оставляет сообщение, электронное письмо с автоматическим ответом будет автоматически отправлено на адрес электронной почты, указанный клиентом.',
  ...
  'plugin.sendmail.auto-reply.message.description': 'Пожалуйста, заполните содержание автоматического ответа',
  ...
  'plugin.sendmail.send-type.description': 'После выбора электронные письма будут отправляться по выбранному сценарию.',
  ...
  'plugin.sendmail.tips': 'Напоминания по электронной почте могут отправлять сообщения с веб-сайта на ваш почтовый ящик по электронной почте.',
  'plugin.sendmail.test.send': 'Отправить тестовое письмо',
  'plugin.sitemap.tips1': 'В настоящее время все основные поисковые системы поддерживают файлы карты сайта в формате txt при отправке файлов карты сайта, а размер файлов карты сайта txt меньше, чем у файлов карты сайта xml. Поэтому рекомендуется использовать карты сайта формата txt.',
  'plugin.sitemap.tips2': 'Поскольку отправка карты сайта каждой поисковой системы ограничена 50 000 элементами или размером 10 МБ, эта функция карты сайта сгенерирует файл карты сайта с 50 000 элементов.',
  ...
  'plugin.sitemap.exculde-module.description': 'Если вы хотите исключить определенные модели документов, вы можете выбрать это здесь.',
  'plugin.sitemap.exculde-category': 'Исключенные категории',
  'plugin.sitemap.exculde-category.description': 'Если вы хотите исключить определенные категории, вы можете выбрать здесь',
  'plugin.sitemap.exculde-page': 'Исключенная отдельная страница',
  'plugin.sitemap.exculde-page.description': 'Если вы хотите исключить определенные отдельные страницы, вы можете выбрать здесь',
  'plugin.sitemap.action': 'ручная операция',
  'plugin.sitemap.action.tips': 'Совет: После изменения конфигурации Sitemap создайте его вручную, чтобы конфигурация вступила в силу.',
  ...
  'plugin.storage.tips': 'Переключение методов хранения ресурсов не приводит к автоматической синхронизации ранее загруженных ресурсов. Обычно не рекомендуется переключать методы хранения во время использования.',
  ...
  'plugin.storage.keep-local.description': 'При использовании облачного хранилища вы можете сохранить локальные архивы.',
  ...
  'plugin.timefactor.end-day.description': 'Если вы заполните 0, это означает, что он будет обновлен до текущего дня.',
  ...
  'plugin.timefactor.republish.description': 'При обновлении документа повторно отправьте его в поисковые системы.',
  ...
  'plugin.timefactor.daily-limit.description': 'После настройки публиковать указанное количество статей из черновика каждый день, значение по умолчанию — 100.',
  ...
  'plugin.timefactor.start-time.description': 'Например: 8, тогда каждый день начинается в 8 часов.',
  'plugin.timefactor.end-time': 'Время окончания',
  'plugin.timefactor.end-time.placeholder': 'Такие как: 18',
  'plugin.timefactor.end-time.description': 'Если вы вставите 0, это значит, что оно закончится в 23:00.',
  'plugin.timefactor.module': 'Открытая модель',
  'plugin.timefactor.category': 'Категории, которые не участвуют в обновлениях',
  'plugin.timefactor.category.placeholder': 'Если вы хотите исключить определенные категории, вы можете выбрать здесь',
  ...
  'plugin.titleimage.open.description': 'Если этот параметр включен, то если в документе нет изображения, изображение, содержащее заголовок документа, будет автоматически создано в качестве миниатюры документа.',
  'plugin.titleimage.draw-sub': 'Создавать ли вторичное изображение заголовка для документа',
  'plugin.titleimage.draw-sub.description': 'После его включения, если в документе нет изображения, изображение будет автоматически сгенерировано для тега h2 документа и вставлено в документ.',
  'plugin.titleimage.size': 'Создать размер изображения',
  'plugin.titleimage.width': 'Ширина пикселя',
  'plugin.titleimage.width.placeholder': 'Создавать ли текст с 800 дополнительными заголовками и изображениями по умолчанию.',
  ...
  'plugin.titleimage.noise.description': 'Работает только в том случае, если используется фон по умолчанию.',
  'plugin.titleimage.bg-image': 'пользовательский фон',
  'plugin.titleimage.bg-image.description': 'Вы можете настроить фон. Если вы не загрузите собственный фон, система автоматически создаст сплошной цветной фон.',
  ...
  'plugin.transfer.token.required': 'Пожалуйста, заполните токен связи, который может быть любым символом.',
  ...
  'plugin.transfer.tips': 'В настоящее время поддерживается перенос содержимого веб-сайтов DedeCMS/WordPress/PbootCMS/EmpireCMS на anqicms.',
  ...
  'plugin.transfer.step2.tips': 'Пожалуйста, загрузите загруженный файл в корневой каталог вашего сайта. После загрузки и размещения его в корневом каталоге вашего веб-сайта нажмите «Далее», чтобы продолжить.',
  'plugin.transfer.step3.tips': 'Для каждого веб-сайта можно настроить только один токен. Если появится сообщение об ошибке, вручную удалите файл anqicms.config.php в корневом каталоге веб-сайта, чтобы настроить его заново.',
  ...
  'plugin.transfer.step4.tips': 'По умолчанию переносится все. Вы можете перенести только определенные части.',
  ...
  'plugin.user.password.description': 'Если вам необходимо изменить пароль для этого пользователя, введите его здесь, не менее 6 символов.',
  ...
  'plugin.user.expire.description': 'По истечении срока действия группа пользователей вернется к первой группе.',
  ...
  'plugin.watermark.generate.confirm': 'Вы уверены, что хотите добавить водяной знак ко всем изображениям в библиотеке изображений?',
  'plugin.watermark.generate.content': 'Изображения с водяными знаками не будут добавлены повторно.',
  'plugin.watermark.open': 'Включить ли водяной знак',
  'plugin.watermark.open.no': 'закрытие',
  'plugin.watermark.open.yes': 'включать',
  'plugin.watermark.open.description': 'Если эта функция включена, водяные знаки будут автоматически добавляться к загруженным изображениям.',
  ...
  'plugin.watermark.batch-add': 'Пакетное добавление водяных знаков к изображениям в библиотеке изображений.',
  'plugin.watermark.min-size': 'Минимальное изображение водяного знака',
  'plugin.watermark.min-size.suffix': 'Пиксель',
  'plugin.watermark.min-size.description': 'Изображения, длина и ширина которых меньше этого размера, не будут добавляться с водяным знаком.',
  ...
  'plugin.weapp.encoding-aes-key.description': 'Если методом шифрования и дешифрования сообщения является текстовый режим, не заполняйте это поле, иначе будет сообщено об ошибке.',
  'plugin.weapp.default': 'Апплет по умолчанию',
  'plugin.weapp.default.tips': 'Мини-программа AnQiCMS по умолчанию также поддерживает мини-программу Baidu Smart, мини-программу WeChat, мини-программу QQ, мини-программу Alipay и мини-программу Toutiao.',
  'plugin.weapp.default.help': 'Помощь по использованию мини программы:',
  'plugin.weapp.default.source': 'Адрес исходного кода мини-программы: https://github.com/fesiong/anqicms-app/releases',
  ...
  'plugin.wechat.menu.submit.content': 'Эта операция синхронизирует вновь установленное меню с сервером WeChat.',
  ...
  'plugin.wechat.menu.value.description': 'Пожалуйста, заполните текст текстового меню и URL-адрес меню ссылок, не более 128 символов.',
  'plugin.wechat.menu': 'Меню WeChat',
  'plugin.wechat.menu.tips': 'Примечание. Для каждого меню первого уровня существует максимум 3 меню первого уровня и максимум 5 меню второго уровня.',
  ...
  'plugin.wechat.reply.default.description': 'Если после выбора в качестве ответа по умолчанию ключевое слово не соответствует, на этот контент будет дан ответ.',
  ...
  'plugin.wechat.verify-key.description': 'Пользователи могут получить код подтверждения, ответив на это ключевое слово.',
  'plugin.wechat.verify-msg': 'Шаблон информации о коде подтверждения',
  'plugin.wechat.verify-msg.placeholder': 'По умолчанию: код подтверждения: {code}, действителен в течение 30 минут.',
