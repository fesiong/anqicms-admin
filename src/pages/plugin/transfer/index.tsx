    ...
    <PageContainer>
      <Card title="网站内容迁移">
        <Alert
          ...
          message="目前支持 DedeCMS 和 WordPress 的网站内容迁移到 anqicms 中。"
        />
                ...
                <>
                  <div>
                    <div style={{ marginBottom: '30px' }}>
                      <Alert message="迁移过程中，请不要刷新本页面。" />
                    </div>
                    <p>需要转移的站点：{task.base_url}</p>
                    <p>
                      当前任务状态：
                      {task.status == 2 ? '已完成' : task.status == 1 ? '进行中' : '未开始'}
                    </p>
                    {task.status == 1 && (
                      <p>
                        当前任务进度：正在迁移 {task.current}{' '}
                        {task.last_id > 0 ? '，数据量：' + task.last_id : ''}
                      </p>
                    )}
